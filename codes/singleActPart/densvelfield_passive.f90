        ...
        ! This code is only for passive system
        ! For each 'B'-type particle we calculate the
        ! coarse-grained density and velocity fields
        
        IMPLICIT NONE

        INTEGER, PARAMETER :: N = 5040, NB=1764
        INTEGER, PARAMETER :: FRAMEST = 2
        INTEGER, PARAMETER :: FRAMEND = 1000
        INTEGER, PARAMETER :: TGAP = 2
        INTEGER, PARAMETER :: NGRID = 30
         REAL*8, PARAMETER :: L0 = 9.0D1, R0 = 1.8D0
         ...
         REAL*8, PARAMETER :: dX = L0/(1.D0*NGRID), dY = dX
                   ...
                   INTEGER :: PT, JC(1:NB), IJC, COUNTJC, JC0
 ...
 20    ENDDO !K
       ...
       DRIFTY = DRIFTY/(NGRID*NGRID) ! = 0.D0

       DO I0 = 1, NGRID
       DO J0 = 1, NGRID
       RHOAV(I0,J0) = RHOAV(I0,J0)/(1.D0*NFRAME)               !(1.D0*(FRAMEND + 1 - FRAMEST))
       ...
       STOP
       END PROGRAM DENSVELFIELD_PASSIVE
